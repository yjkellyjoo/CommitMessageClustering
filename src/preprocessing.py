...
if __name__ == "__main__":
    ...
    #     vuln = preprocessor.transform()
    #     print(vuln)

    # request query to not vulnerable commits
    query = "SELECT message, REPONAME, COMMITID FROM git.TB_NOTVULN;"
    results = execute_read_query(connection, query)
    ...
    for result in results:
        ...
        # tokenize the message and save into pickled file
        preprocessor = Preprocessor(message, reponame, commitid, "../corpus/not_vulnerable")
        notvuln = preprocessor.transform()
        print(notvuln)
